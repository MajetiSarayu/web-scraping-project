...
def scrape(fields):
    ...
    if fields:
        ...
        prompt =  f"scrape data about the fields"
...
def ui():
    ...
    with st.sidebar:
        ...
        if st.button("Scrape"):
            # not really scraping the everything from the urlright now but just to make sure
            #  that dynamic listing model works we are querying the gemini model
            response = scrape(fields)
    if response:
        st.success("Data Scraped successfully")
        st.dataframe(response["listings"])
